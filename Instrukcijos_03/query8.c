...
int main(int argc, char *argv[]) {
    ...
    if (db){                            
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            for (int j = 0; j < s.load; ++j){ // looks for "Calculus in student's courses array
                if(strcmp(s.courses[j], "Calculus") == 0)
                {
                    for (int k = 0; k < s.load; ++k) // looks for "Calculus in student's courses array
                    {
                        if(strcmp(s.courses[k], "Algebra") == 0)
                            found++;
                            break;
                    }
                }
            }
            if(found > 0){ // *** first filter, conditions on the student
